...
class Main {
    ...
    public static void printFilteredAccounts(List<Account> accounts) {
        List<Account> nonEmptyAccounts = filter(accounts, n -> n.getBalance() > 0);
        List<Account> accountsWithTooMuchMoney = filter(accounts, n -> !n.locked && n.getBalance() >= 100_000_000L);
